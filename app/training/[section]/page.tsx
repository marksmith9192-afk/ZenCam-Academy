...
export default async function SectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  return <TrainingShell activeSection={section.slug}>
    <section className={`section-hero ${section.color}`}>
      ...
      <div><span>ZenduONE Academy / {section.title}</span><h1>{section.title} training</h1><p>{section.description}</p><small>{available} {available === 1 ? "lesson" : "lessons"}</small></div>
    </section>
